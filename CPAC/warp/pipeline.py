...
def ants_apply_warps_asl_mni(
    ...
    # apply ants warps
    apply_ants_warp_asl_mni = \
        create_wf_apply_ants_warp(name='apply_ants_warp_%s_%d' % (func_name, num_strat),
                                  ants_threads=int(num_ants_cores))

    apply_ants_warp_asl_mni.inputs.inputspec.reference_image = standard
    apply_ants_warp_asl_mni.inputs.inputspec.dimension = 3
    apply_ants_warp_asl_mni.inputs.inputspec.interpolation = interp

    # input_image_type:
    # (0 or 1 or 2 or 3)
    # Option specifying the input image type of scalar
    # (default), vector, tensor, or time series.
    apply_ants_warp_asl_mni.inputs.inputspec. \
        input_image_type = input_image_type

    # convert the .mat from linear Func->Anat to
    # ANTS format
    node, out_file = strat['asl_to_anat_linear_xfm']
    workflow.connect(node, out_file, fsl_to_itk_func_mni,
                     'inputspec.affine_file')
    ...
    strat.update_resource_pool({
        func_name: (apply_ants_warp_asl_mni, 'outputspec.output_image')
    })

    strat.append_name(apply_ants_warp_asl_mni.name)

    return apply_ants_warp_asl_mni
